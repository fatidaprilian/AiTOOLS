...
class DocumentConversionController extends Controller
{
    ...
    public function convertToPdf(Request $request)
    {
        ...
        if (!$apiKey) {
            ...
            return response()->json([
                'status' => 'error',
                'message' => 'Kunci API layanan konversi tidak dikonfigurasi.'
            ], 500);
        }
        ...
        try {
            ...
            $jobResponse = Http::withoutVerifying()
            ...
            if (!$jobResponse->successful() || !isset($jobResponse->json()['data']['id'])) {
                ...
                return response()->json([
                    'status' => 'error',
                    'message' => 'Gagal membuat job konversi di CloudConvert.'
                ], 500);
            }
            ...
            if (!$uploadTaskUrl) {
                ...
                return response()->json([
                    'status' => 'error',
                    'message' => 'Gagal mendapatkan URL untuk mengunggah file ke job.'
                ], 500);
            }
            ...
            $fileUploadResponse = Http::withoutVerifying()
                ->asMultipart()
            ...
            if (!$fileUploadResponse->successful()) {
                ...
                ToolUsageLog::create([
                    ...
                    'status' => 'error',
                    'details' => json_encode([
                        'error_type' => 'CloudConvert API',
                        'message' => 'Gagal mengunggah file ke job.',
                        'job_id' => $jobId,
                        'status_code' => $fileUploadResponse->status(),
                        'api_response_body' => substr($errorBodyText, 0, 500)
                    ]),
                    'processing_time_ms' => $processingTimeMs,
                ]);
                return response()->json([
                    'status' => 'error',
                    'message' => 'Gagal mengunggah file ke layanan konversi. Status: ' . $fileUploadResponse->status() . ' Respons: ' . substr($errorBodyText, 0, 200)
                ], 500);
            }
                    ...
                    if ($statusData['status'] === 'finished') {
                        ...
                        if (!$pdfUrl) {
                            ...
                            ToolUsageLog::create([
                                ...
                                'status' => 'error',
                                'details' => json_encode([
                                    'error_type' => 'CloudConvert Job Logic',
                                    'message' => 'Job selesai tapi URL ekspor tidak ditemukan.',
                                    'job_id' => $jobId,
                            ...
                            return response()->json([
                                'status' => 'error',
                                'message' => 'Gagal mendapatkan URL hasil dari CloudConvert setelah job selesai.'
                            ], 500);
                        }
                        ...
                        ToolUsageLog::create([
                            ...
                            'status' => 'error',
                            'details' => json_encode([
                                'error_type' => 'CloudConvert Job Error',
                                'message' => $errorMessage,
                                'job_id' => $jobId,
                                'job_status_data' => $statusData
                            ]),
                            'processing_time_ms' => $processingTimeMs,
                        ]);
                        return response()->json([
            ...
            if (!$pdfUrl) {
                ...
                ToolUsageLog::create([
                    ...
                    'status' => 'error',
                    'details' => json_encode([
                        'error_type' => 'CloudConvert Timeout/No URL',
                        'message' => 'Konversi memakan waktu terlalu lama atau gagal mendapatkan URL hasil.',
                        'job_id' => $jobId
                    ]),
                    'processing_time_ms' => $processingTimeMs,
                ]);
                return response()->json([
                    'status' => 'error',
                    'message' => 'Konversi memakan waktu terlalu lama atau gagal mendapatkan URL hasil dari CloudConvert.'
                ], 500);
            }
            ...
            return response()->json([
                'status' => 'success',
                'pdf_url' => $pdfUrl,
                'filename' => $finalOutputPdfName,
            ]);
        } catch (\Exception $e) {
            Log::error('Word to PDF Conversion Exception: ' . $e->getMessage(), [
            ...
            ToolUsageLog::create([
                ...
                'details' => json_encode([
                    ...
                    'message' => Str::limit($e->getMessage(), 255),
                    'file' => $e->getFile(),
                    'line' => $e->getLine()
                ]),
                'processing_time_ms' => $processingTimeMs,
            ...
            return response()->json([
                'status' => 'error',
                'message' => 'Terjadi kesalahan internal saat memproses: ' . Str::limit($e->getMessage(), 100)
            ], 500);
        }
    }
}
